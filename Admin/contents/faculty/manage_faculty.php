...
if(isset($_GET['id'])){
	$qry = $conn->query("SELECT * FROM table_faculty where id={$_GET['id']}")->fetch_array();
	foreach($qry as $k => $v){
		$$k = $v;
	}
}
...
<div class="container-fluid">
	<form action="" id="manage-faculty" novalidate>
				...
				$.ajax({
					...
					success:function(resp){
						if(resp == 1){
							...
							$('#msg').html('<div class="alert alert-danger"><i class="fa fa-exclamation-triangle"></i> Faculty Staff already exist.</div>')
							end_load()
						}
					}
				});
